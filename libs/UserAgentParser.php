...
function parse_user_agent( $u_agent = null ) {
	...
	if( !$u_agent ) {
		return array( 'device' => $device ?: 'unknown', 'browser' => $browser ?: 'unknown', 'os' => $platform ?: 'unknown' );
	}
	...
	// Added Device detection - AvalonRychmon
	$device = array(
	    'desktop' => array('Windows','Linux','Macintosh','Chrome OS'),
        'mobile' => array('Android','iPhone','iPad','iPod Touch','Windows Phone OS','Kindle','Kindle Fire','BlackBerry','Playbook','Tizen'),
        'console' => array('Oculus','Nintendo 3DS','New Nintendo 3DS','Nintendo Wii','Nintendo WiiU','PlayStation 3','PlayStation 4','PlayStation Vita','Xbox 360','Xbox One')
    );
        
	#$browser = $browser.' '.$version;
	foreach($device as $key => $value){
	    if(in_array($platform, $value)){
	        $device = $key;
	    }
	}
	// End of Edit
